<?php //netteloader=DibiPcreException,IDataSource,IDibiProfiler,IDibiDriver,IDibiResultDriver,IDibiReflector,DibiDateTime,DibiObject,DibiLazyStorageBase,DibiLazyStorage,DibiException,DibiDriverException,DibiConnection,DibiResult,DibiResultIterator,DibiRow,DibiTranslator,DibiDataSource,DibiFluent,DibiDatabaseInfo,DibiTableInfo,DibiResultInfo,DibiColumnInfo,DibiForeignKeyInfo,DibiIndexInfo,DibiProfiler,DibiVariable,dibi,DibiMySqlReflector,DibiMySqlDriver,DibiMySqliDriver,DibiOdbcDriver,DibiPdoDriver,DibiPostgreDriver,DibiSqliteReflector,DibiSqliteDriver,DibiSqlite3Driver
...
DibiTranslator($this->driver);$profilerCfg=&$config['profiler'];if(is_scalar($profilerCfg)){$profilerCfg=array('run'=>(bool)$profilerCfg,'class'=>strlen($profilerCfg)>1?$profilerCfg:NULL);}if(!empty($profilerCfg['run'])){class_exists('dibi');$class=isset($profilerCfg['class'])?$profilerCfg['class']:'DibiProfiler';if(!class_exists($class)){throw
new
...
DibiLazyStorage(array(__CLASS__,'detectType'));}return
...
__construct($val){parent::__construct($val);}}class
dibi{const
TEXT='s';const
BINARY='bin';const
BOOL='b';const
...
VERSION='1.3-dev';const
REVISION='af6352d released on 2010-11-10';const
ASC='ASC',DESC='DESC';private
static$registry=array();private
static$connection;public
static$substs;private
...
static
function
datetime($time=NULL){return
new
DibiDateTime(is_numeric($time)?date('Y-m-d H:i:s',$time):$time);}public
static
function
date($date=NULL){return
new
DibiDateTime(is_numeric($date)?date('Y-m-d',$date):$date);}public
static
function
addSubst($expr,$subst){self::$substs->$expr=$subst;}public
static
function
removeSubst($expr){if($expr===TRUE){self::$substs=new
DibiLazyStorage(self::$substs->getCallback());}else{unset(self::$substs->$expr);}}public
static
function
setSubstFallback($callback){self::$substs->setCallback($callback);}public
static
function
defaultSubstFallback($expr){return":$expr:";}public
static
function
...
static
function
highlightCallback($matches){if(!empty($matches[1]))return'<em style="color:gray">'.$matches[1].'</em>';if(!empty($matches[2]))return'<strong style="color:red">'.$matches[2].'</strong>';if(!empty($matches[3]))return'<strong style="color:blue">'.$matches[3].'</strong>';if(!empty($matches[4]))return'<strong style="color:green">'.$matches[4].'</strong>';}}dibi::$substs=new
DibiLazyStorage(array('dibi','defaultSubstFallback'));class
DibiMySqlReflector
extends
DibiObject
implements
...
new
DibiDriverException('Driver does not support meta data.');}$row['table']=isset($row['table'])?$row['table']:NULL;$columns[]=array('name'=>$row['name'],'table'=>$row['table'],'nativetype'=>$row['native_type'],'fullname'=>$row['table']?$row['table'].'.'.$row['name']:$row['name'],'vendor'=>$row);}return$columns;}public
function
getResultResource(){return$this->resultSet;}}class
DibiPostgreDriver
extends
...
new
DibiDriverException("PHP extension 'sqlite3' is not loaded.");}}public
function
